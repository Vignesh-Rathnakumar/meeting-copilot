...
def save_meeting_result(meeting_id: str, data: dict):
    ...
    print(f"💾 Saved meeting result to {filename}")
...
def process_meeting(
    ...
    """
    print(f"\n{'='*60}")
    print(f"🚀 Starting meeting processing: {Path(file_path).name}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        if file_ext in [".mp3", ".wav", ".m4a", ".ogg", ".flac"]:
            ...
            print("📄 Step 1: Loading text transcript...")
        ...
        print(f"✅ Transcript ready ({len(labeled_transcript)} characters)\n")
        ...
        print(f"✅ Analysis complete: {len(analysis.get('action_items', []))} action items, {len(analysis.get('decisions', []))} decisions\n")

        # ─────────────────────────────────────────
        # Step 3: Create tasks (if requested)
        # ─────────────────────────────────────────
        if create_tasks and analysis.get("action_items"):
            print("📋 Step 3: Creating tasks from action items...")
            try:
                ...
                print(f"✅ Created {len(tasks_result)} tasks\n")
            except Exception as e:
        ...
        # ─────────────────────────────────────────
        if send_email:
            print("📧 Step 4: Sending email summary...")
            ...
            if not recipients:
                print("⚠️  No recipients specified for email. Skipping.")
            else:
                try:
                    ...
                    if result["email_sent"]:
                        ...
                        print(f"⚠️  Email failed: {email_status.get('error', 'Unknown error')}\n")
                except Exception as e:
        ...
        # ─────────────────────────────────────────
        print("🧠 Step 6: Storing in memory (RAG)...")
        try:
        ...
        print(f"\n{'='*60}")
        print(f"✅ Meeting processing complete! ID: {meeting_id}")
        print(f"{'='*60}\n")
...
if __name__ == "__main__":
    ...
    print("🧪 Testing orchestrator with sample transcript...\n")
